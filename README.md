<div align="center">
  <img src="docs/assets/hero.svg" alt="PortAI DT Mobile — mobile port digital twin" width="100%" />
...
![Data](https://img.shields.io/badge/data-public%20datasets-00a884)
![Dispatch](https://img.shields.io/badge/production%20dispatch-disabled-c84667)

[双语说明 / Bilingual guide](#双语说明--bilingual-guide) · [共享后端契约 / Shared backend](docs/SHARED_BACKEND_CONTRACT.md) · [测试与指标 / Metrics](docs/TEST_AND_METRIC_INDEX.md) · [安全策略 / Security](SECURITY.md)
</div>
...
<table>
  <tr>
    ...
    <th align="center">自动化测试<br /><sub>AUTOMATED TESTS</sub></th>
  </tr>
  <tr>
...
<p align="center">
  <sub>与 Web 端共享同一业务数据和 FastAPI 决策后端；移动端负责人工确认、服务端回执与失效安全，不重复计算业务 KPI。</sub><br />
  <sub>The mobile frontend shares business data and one FastAPI decision service with the Web system, adding human confirmation, server receipts, and fail-safe behavior without recalculating business KPIs.</sub>
</p>
...
PortAI DT Mobile is the Flutter frontend of the dual-frontend port decision system. Together with the Web frontend, it connects to the same [`port-dt-multi`](https://github.com/wenjiayi123/port-dt-multi) FastAPI service. Mobile owns situational awareness, risk review, candidate comparison, human decisions, receipts, and audit replay; Web owns digital-twin modelling, parameter configuration, training/evaluation, and strategy simulation. Both consume the same business benchmark, model registry, and server-side audit records.

![移动端双端共享数据与回执](docs/assets/mobile-shared-evidence-v2.jpg)

移动首页直接读取共享后端身份、系统级 KPI 和 500 项移动端闭环测试结果；移动端不在本地重复计算收益指标。<br>
The mobile home screen reads backend identity, system KPIs, and the results of 500 mobile workflow tests from the shared service; business KPIs are not recalculated on the client.
...
| 层级 / Layer | 已实现能力 / Implemented capability | 实现与测试 / Implementation and tests |
|---|---|---|
...
| 算法与评测 / Algorithms & evaluation | SAC、PPO、TD3、DQN、A2C、TQC 与 MPC；训练不渲染、测试独立<br><sub>Six RL methods plus MPC; headless training with independent testing</sub> | 18 组正式 RL 训练 + 1 组 MPC 基准<br><sub>18 formal RL runs plus one MPC benchmark</sub> |
| 业务基准 / Business benchmark | 52,608 条小时驱动记录、35,064/8,784/8,760 时序切分、2025 全年测试<br><sub>52,608 hourly drivers, 35,064/8,784/8,760 temporal split, and a full 2025 test</sub> | 固定业务报告及数据/配置 SHA-256<br><sub>Pinned report and data/config SHA-256</sub> |
| 人机治理 / Human governance | 移动申请、电脑端异人审批、幂等表态、服务端回执<br><sub>Mobile request, different desktop approver, idempotent decision, server receipt</sub> | 申请人与审批人分离、原子记录、审计前向链<br><sub>Requester/approver separation, atomic records, forward audit chain</sub> |
...
移动端一次核验 `public_us_la_6min_v1`、数据 SHA-256、`port_ops_v2/v3`、37维观测、5维动作、共享算法注册表和已完成实验数量；任一核心项不一致即停止加载策略。当前 V3.2 Web 后端返回 10 个 RL 方法以及 MPC、FCFS 两类控制/规则基线，移动端保留原 6 RL + MPC 核心兼容合同并校验扩展方法是否登记。<br>
The client verifies the dataset, SHA-256, `port_ops_v2/v3`, 37-D observations, five actions, the shared method registry, and completed-run counts as one compatibility contract. Any mismatch stops policy loading. The current V3.2 Web backend exposes ten RL methods plus MPC and FCFS while retaining the original six-RL-plus-MPC compatibility core.
...
![移动端公开数据与训练页面](docs/assets/mobile-public-data-evidence-v2.jpg)
...
只用于算法接线研究；它与上述系统业务基准相互独立，不混合计算。

The optional standalone backend bundles a separate historical NOAA/MarineCadastre AIS sample near Long Beach for algorithm-wiring research only. It is independent of the system business benchmark and is never mixed into its calculations.
...
5. 移动端只需更换 `API_BASE_URL` 与 `RL_DATASET_ID` 场景参数，会重新核验数据、环境、算法和接口合同。 / Change only the `API_BASE_URL` and `RL_DATASET_ID` scenario parameters; mobile revalidates the dataset, environment, algorithms, and service contract.
...
指标、测试与双端接口的对应关系见
[`docs/TEST_AND_METRIC_INDEX.md`](docs/TEST_AND_METRIC_INDEX.md)。

See [`docs/TEST_AND_METRIC_INDEX.md`](docs/TEST_AND_METRIC_INDEX.md) for the mapping among metrics, tests, and dual-frontend interfaces.
...
Current mobile baseline: Flutter analysis and 25 client tests pass. The shared backend separately verifies six RL methods plus MPC and 500 mobile closed-loop operations. The standalone AIS backend’s short smoke run checks wiring only.

### 现场集成与安全 / Site integration and safety
...
scripts/                     启动、导入与测试工具 / launch, import, and test tools

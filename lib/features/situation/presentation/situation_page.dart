  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        const SizedBox(height: 12),
        _TrendProjectionCard(points: snapshot.trendPoints),
      ],
    );
  }
}
...
class _TrendProjectionCard extends StatelessWidget {
  const _TrendProjectionCard({required this.points});
  ...
  Widget build(BuildContext context) {
    ...
    return AppSectionCard(
      title: '历史密度序列',
      subtitle: '后端返回的最近观测点 · 非未来预测',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('交通密度历史点', style: theme.textTheme.titleMedium),
          const SizedBox(height: 10),

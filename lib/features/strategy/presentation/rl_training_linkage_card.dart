  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(
      children: [
        ...
        Expanded(
          child: _ConfigItem(
            label: config.algorithm == 'los_pid' ? '校准试验' : '训练步数',
            value: _compactInt(config.totalSteps),
          ),
        ),
        Expanded(
          child: _ConfigItem(
            label: '批大小',
            value: config.algorithm == 'los_pid'
                ? '不适用'
                : '${config.batchSize}',
          ),
        ),
          ...
          children: [
            Expanded(
              child: _ProgressMetric(
                label: state.config.algorithm == 'los_pid' ? '校准试验' : '步数',
                value:
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 14),
              _SheetChoiceRow(
                title: config.algorithm == 'los_pid' ? 'PID增益校准试验数' : '训练步数',
                values: config.algorithm == 'los_pid'
                    ? const [5, 15, 30]
                    : const [5000, 20000, 100000],
                selected: config.totalSteps,
                label: (value) => _compactInt(value),
                onSelected: (value) => setSheetState(
                  () => config = config.copyWith(totalSteps: value),
                ),
              ),
              const SizedBox(height: 12),
              if (config.algorithm != 'los_pid')
                _SheetChoiceRow(
                  title: '批大小',
